...
export default function SingleCard({ ids = { ind: 0, index: 0 }, card = {} }) {
    ...
    return (
        <>
            {
                card ?                
                <div ref={cardRef} style={{ zIndex: isFocused && "10" }} className="relative inline-block transition-transform z-1">
                    <div style={{ transition: `clip-path .1s ease-in ${isFocused ? "0s" : ".2s"}`, clipPath: `${isFocused ? "polygon(0 0, 100% 0, 100% 100%, 0% 100%)" : "polygon(0 100%, 100% 100%, 100% 100%, 0% 100%)"}` }} className="z-10 flex items-center bg-[rgba(0,0,0,0.1)] rounded-md justify-center absolute inset-0">
                        <button onClick={dropBtnClickHander} className="px-4 font-medium rounded-md py-2 bg-[var(--terColor)] text-[var(--mainColor)]">Drop</button>
                    </div>
                    <button onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} className={`max-w-28 pointer-events-${isMyTurn && playerAction === "Drop" ? "auto" : "none"}`}>
